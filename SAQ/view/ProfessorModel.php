...
<!-- this is a button that create  a qr code-->
  ...
  <a href='LoginPage.html' class = "block">Logout</button></a>
  <p> &nbsp;&nbsp;&nbsp;</p>
...
<?Php 
    ...
    while($row = $result->fetch_assoc()) {
	   ...
	   if($row["Attendance"] == 1){
	echo '<div class="card bg-success text-white" style="width: 18rem; id = "'.$row["studentID"].'">';
	echo '<div class= "card-body">';
	print $row["studentFname"]." ".$row["studentLname"]." ".$row["studentID"];
	echo '</div>';
	echo '</div>';
	echo '<p> &nbsp;&nbsp;&nbsp;</p>';
	   }
